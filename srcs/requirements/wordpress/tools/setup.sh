...
if [ ! -f "$WP_PATH/wp-config.php" ]; then
    ...
    wp core install --url=${DOMAIN_NAME} --title="Yippieee" --admin_user=${WORDPRESS_ADMIN_USER} --admin_password=${WORDPRESS_ADMIN_PASSWORD} --admin_email=${WORDPRESS_ADMIN_EMAIL} --allow-root
else
...
#- - - - - - - - - - - - - - - -
# (line 37)
# 2>&1 redirige standard error su standard output
#- - - - - - - - - - - - - - - -
# (line 49)
# fa partire PHP-FPM (FastCGI Process Manager) in foreground (-F), sostituscse
# il processo corrente (setup.sh) con php-fpm per gestire le richieste PHP
#- - - - - - - - - - - - - - - -