...
if [ ! -f "$WP_PATH/wp-config.php" ]; then
    ...
    wp core install --url=mfanelli.42.fr --title="Non funziona una minchia" --admin_user=${WORDPRESS_ADMIN_USER} --admin_password=${WORDPRESS_ADMIN_PASSWORD} --admin_email=${WORDPRESS_ADMIN_EMAIL} --allow-root
else
